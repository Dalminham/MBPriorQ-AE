...
from mbpriorq_ae import GlobalEBW, GlobalVMBProfiler
...
def parse_args():
    ...
    parser.add_argument(
        "--weight-source",
        default="auto",
        choices=("auto", "none", "online", "prequant"),
    ...
    parser.add_argument(
        "--kv-cache-method",
        default="disabled",
        choices=("disabled", "bf16", "nvfp4", "mbpriorq"),
    )
...
def _resolve_weight_source(args) -> tuple[str, dict | None]:
    ...
    if source == "none":
        if args.imatrix:
            raise ValueError("Activation-only evaluation does not consume an imatrix")
        return source, None
    if source == "prequant":
...
def main():
    ...
    if args.kv_cache_method != "disabled":
        if args.method != "bf16" or args.backend != "full_gpu":
            raise ValueError("KV-cache rows require BF16 weights/activations and full_gpu")
        if args.batch_size != 1:
            raise ValueError("Table 8 KV-cache PPL uses batch size 1")
    ...
    GlobalEBW.reset()
    GlobalVMBProfiler.configure(
        enabled=bool(args.vmb_profile_output),
        output_path=args.vmb_profile_output,
        metadata={
            "model_name": args.model_key or args.model,
    ...
    if args.backend in {"full_gpu", "by_layer"}:
        ...
        if args.backend == "by_layer":
            ...
            cache_factory = None
            if args.kv_cache_method in {"nvfp4", "mbpriorq"}:
                cache_factory = lambda: build_kv_cache(
                    model.config,
                    args.kv_cache_method,
                    refined_block_size=args.refined_block_size,
                )
            ppl, windows, total_nll = paper_compatible_kv_cache_ppl(
                model,
                input_ids,
                sequence_length=args.sequence_length,
                num_samples=args.num_samples,
    ...
    profile_path = GlobalVMBProfiler.write_csv()
    kv_cache_ebw_summary = None
    if args.kv_cache_method == "nvfp4":
        kv_cache_ebw_summary = {
            "method": "nvfp4",
            "payload_ebw": 4.0,
            "scale_ebw": 0.5,
            "effective_ebw": 4.5,
        }
    elif args.kv_cache_method == "mbpriorq":
        kv_cache_ebw_summary = GlobalEBW.summarize("kv_cache")
    result = {
        "method": args.method,
        "model": args.model,
        "dataset": args.dataset,
        ...
        "kv_cache_ebw_summary": kv_cache_ebw_summary,
