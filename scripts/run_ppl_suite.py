#!/usr/bin/env python3
"""Run the paper's BF16/MBPriorQ model-family PPL matrix."""
...
DEFAULT_MANIFEST = ROOT / "experiments/model_family_ppl/models.json"
DEFAULT_IMATRIX = ROOT / "data/imatrix/Qwen_Qwen3-0.6B.imatrix"


def parse_args():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--suite", default="paper16", choices=("paper16", "table2_19"))
    ...
    parser.add_argument("--output-root", default=str(ROOT / "local_runs/model_family_ppl"))
...
def _load_models(path: str, suite: str, only: set[str]) -> list[dict]:
    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    models = [entry for entry in payload["models"] if suite in entry["suites"]]
    if only:
        unknown = only - {entry["key"] for entry in models}
        if unknown:
            raise ValueError(f"Unknown or out-of-suite model keys: {sorted(unknown)}")
        models = [entry for entry in models if entry["key"] in only]
    expected_count = 16 if suite == "paper16" else 19
    if not only and len(models) != expected_count:
        raise ValueError(f"Suite {suite} contains {len(models)} models, expected {expected_count}")
    return models
...
def main():
    args = parse_args()
    models = _load_models(args.manifest, args.suite, set(args.only))
